// Package app coordinates the local web app server process lifecycle.
package app
...
const (
	defaultListenAddress    = "0.0.0.0:8766"
	staleProcessStopTimeout = 5 * time.Second
)

// Run starts, stops, or reconnects to the server according to arguments.
...
func shutdownServices(stopBackends func() error, shutdownHTTP func(context.Context) error) error {
	...
	defer cancel()
	return errors.Join(backendErr, shutdownHTTP(shutdownContext))
}
